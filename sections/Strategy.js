import React from 'react'
import Image from 'next/image'
import DummyFood from "@/assets/images/dummy_foodcription.png"
import Step2Grocrey from "@/assets/images/step2_grocery.svg"
import Step6Goal from "@/assets/images/step6_goal.svg"
import Step4Orders from "@/assets/images/step4_orders.svg"
import Write from "@/assets/images/write.svg"
import Step3Yoga from "@/assets/images/step3_yoga.svg"
import Meals from "@/assets/images/step5_meals.svg"

const Strategy = () => {
    return (
        <div className='flex justify-center items-center py-10'>
            <div className='flex flex-col items-center w-full max-h-[1200px]'>
                <div className='flex flex-col justify-center items-center'>
                    <h1 className='text-3xl font-medium text-gray-600'>How It Works</h1>
                    <div className='font-normal text-gray-700'>
                        Unraveling the Mystery of Sustainable Wellness: Explore Our Tailored Strategies for a Healthier You
                    </div>
                </div>
                <div className='flex py-5 w-11/12 mx-auto '>
                    <div className='flex flex-col max-w-[100%] justify-evenly'>
                        <div className='flex rounded-l-full bg-gradient-to-b from-[#FFF] via-[#FFF] to-[#F5F5F5] w-full items-center justify-between'>
                            <div className='flex justify-start w-full'>
                                <Image src={Write} alt="write" width={100} height={100} />
                                <div className='flex flex-col w-[50%]'>
                                    <h2 className='font-extrabold text-4xl text-[#64EDCC]'>01</h2>
                                    <p className=''>Enter your details - Get FoodCription</p>
                                    <div className='flex flex-row justify-between items-center w-[40%]'>
                                        <div className='bg-[#64EDCC] w-2 h-2 rounded-full'></div>
                                        <div className='bg-[#92ADCC] w-1 h-1 rounded-full'></div>
                                        <div className='bg-[#FF965A] w-1 h-1 rounded-full'></div>
                                        <div className='bg-[#A4D221] w-1 h-1 rounded-full'></div>
                                        <div className='bg-[#F6DD5E] w-1 h-1 rounded-full'></div>
                                        <div className='bg-[#FB76BE] w-1 h-1 rounded-full'></div>
                                    </div>
                                </div>
                            </div>
                            <div className='flex flex-col h-max justify-between w-[5%] items-center'>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                            </div>
                        </div>

                        <div className='flex rounded-l-full bg-gradient-to-b from-[#FFF] via-[#FFF] to-[#F5F5F5] w-full items-center justify-between'>
                            <div className='flex justify-start w-full'>
                                <Image src={Step3Yoga} alt="write" width={100} height={100} />
                                <div className='flex flex-col w-[50%]'>
                                    <h2 className='font-extrabold text-4xl text-[#FF965A]'>03</h2>
                                    <p className=''>Yoga, Exercise, Meditation Advice</p>
                                    <div className='flex flex-row justify-between items-center w-[40%]'>
                                        <div className='bg-[#64EDCC] w-1 h-1 rounded-full'></div>
                                        <div className='bg-[#92ADCC] w-1 h-1 rounded-full'></div>
                                        <div className='bg-[#FF965A] w-2 h-2 rounded-full'></div>
                                        <div className='bg-[#A4D221] w-1 h-1 rounded-full'></div>
                                        <div className='bg-[#F6DD5E] w-1 h-1 rounded-full'></div>
                                        <div className='bg-[#FB76BE] w-1 h-1 rounded-full'></div>
                                    </div>
                                </div>
                            </div>
                            <div className='flex flex-col h-max justify-between w-[5%] items-center'>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                            </div>

                        </div>

                        <div className='flex rounded-l-full bg-gradient-to-b from-[#FFF] via-[#FFF] to-[#F5F5F5] w-full items-center justify-between'>
                            <div className='flex justify-start w-full'>
                                <Image src={Meals} alt="write" width={100} height={100} />
                                <div className='flex flex-col w-[50%]'>
                                    <h2 className='font-extrabold text-4xl text-[#F6DD5E]'>05</h2>
                                    <p className=''>Option to buy ready healthy meals</p>
                                    <div className='flex flex-row justify-between items-center w-[40%]'>
                                        <div className='bg-[#64EDCC] w-1 h-1 rounded-full'></div>
                                        <div className='bg-[#92ADCC] w-1 h-1 rounded-full'></div>
                                        <div className='bg-[#FF965A] w-1 h-1 rounded-full'></div>
                                        <div className='bg-[#A4D221] w-1 h-1 rounded-full'></div>
                                        <div className='bg-[#F6DD5E] w-2 h-2 rounded-full'></div>
                                        <div className='bg-[#FB76BE] w-1 h-1 rounded-full'></div>
                                    </div>
                                </div>
                            </div>
                            <div className='flex flex-col h-max justify-between w-[5%] items-center'>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                            </div>
                        </div>
                    </div>
                    <div className='w-[30%] px-3  mx-10'>
                        <Image src={DummyFood} alt='dummy_foodcription' width={420} height={850} />
                    </div>
                    <div className='flex flex-col max-w-[100%] w-[353.65px] justify-evenly'>
                        <div className='flex rounded-r-full bg-gradient-to-b from-[#FFF] via-[#FFF] to-[#F5F5F5] w-full items-center justify-between'>
                            <div className='flex flex-col h-max justify-between w-[5%] items-center'>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                            </div>
                            <div className='flex justify-start w-full'>
                                <Image src={Step2Grocrey} alt="write" width={100} height={100} />
                                <div className='flex flex-col w-[50%]'>
                                    <h2 className='font-extrabold text-4xl text-[#92ADCC]'>02</h2>
                                    <p className=''>Get personalized Grocery List</p>
                                    <div className='flex flex-row justify-between items-center w-[40%]'>
                                        <div className='bg-[#64EDCC] w-1 h-1 rounded-full'></div>
                                        <div className='bg-[#92ADCC] w-2 h-2 rounded-full'></div>
                                        <div className='bg-[#FF965A] w-1 h-1 rounded-full'></div>
                                        <div className='bg-[#A4D221] w-1 h-1 rounded-full'></div>
                                        <div className='bg-[#F6DD5E] w-1 h-1 rounded-full'></div>
                                        <div className='bg-[#FB76BE] w-1 h-1 rounded-full'></div>
                                    </div>
                                </div>
                            </div>

                        </div>

                        <div className='flex rounded-r-full bg-gradient-to-b from-[#FFF] via-[#FFF] to-[#F5F5F5] w-full items-center justify-between'>
                            <div className='flex flex-col h-max justify-between w-[5%] items-center'>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                            </div>
                            <div className='flex justify-start w-full'>
                                <Image src={Step4Orders} alt="write" width={100} height={100} />
                                <div className='flex flex-col w-[50%]'>
                                    <h2 className='font-extrabold text-4xl text-[#A4D221]'>03</h2>
                                    <p className=''>Grocery Buying option from our health partners</p>
                                    <div className='flex flex-row justify-between items-center w-[40%]'>
                                        <div className='bg-[#64EDCC] w-1 h-1 rounded-full'></div>
                                        <div className='bg-[#92ADCC] w-1 h-1 rounded-full'></div>
                                        <div className='bg-[#FF965A] w-1 h-1 rounded-full'></div>
                                        <div className='bg-[#A4D221] w-2 h-2 rounded-full'></div>
                                        <div className='bg-[#F6DD5E] w-1 h-1 rounded-full'></div>
                                        <div className='bg-[#FB76BE] w-1 h-1 rounded-full'></div>
                                    </div>
                                </div>
                            </div>


                        </div>

                        <div className='flex rounded-r-full bg-gradient-to-b from-[#FFF] via-[#FFF] to-[#F5F5F5] w-full items-center justify-between'>
                            <div className='flex flex-col h-max justify-between w-[5%] items-center'>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                                <div className='bg-gradient-to-r from-[#EAEEF0] to-[#8F8F8F] w-1 h-1 rounded-full mb-1'></div>
                            </div>
                            <div className='flex justify-start w-full'>
                                <Image src={Step6Goal} alt="write" width={100} height={100} />
                                <div className='flex flex-col w-[50%]'>
                                    <h2 className='font-extrabold text-4xl text-[#FB76BE]'>05</h2>
                                    <p className=''>Health Goals Fulfilled in the most convenient way</p>
                                    <div className='flex flex-row justify-between items-center w-[40%]'>
                                        <div className='bg-[#64EDCC] w-1 h-1 rounded-full'></div>
                                        <div className='bg-[#92ADCC] w-1 h-1 rounded-full'></div>
                                        <div className='bg-[#FF965A] w-1 h-1 rounded-full'></div>
                                        <div className='bg-[#A4D221] w-1 h-1 rounded-full'></div>
                                        <div className='bg-[#F6DD5E] w-1 h-1 rounded-full'></div>
                                        <div className='bg-[#FB76BE] w-2 h-2 rounded-full'></div>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                </div>

            </div>

        </div>
    )
}

export default Strategy
